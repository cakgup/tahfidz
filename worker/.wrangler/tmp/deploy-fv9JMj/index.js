...
async function createSession(env, userId) {
  ...
  await env.DB.prepare("INSERT INTO sessions (id, user_id, token_hash, expires_at, created_at) VALUES (?, ?, ?, ?, ?)").bind(crypto.randomUUID(), userId, tokenHash, addDays(30), nowIso()).run();
  return token;
}
...
async function requireAuth(request, env) {
  ...
  const row = await env.DB.prepare(`SELECT u.id, u.name, u.email, u.role, u.status, s.id AS session_id
    ...
    WHERE s.token_hash = ? AND s.expires_at > ? AND u.status = 'active'`).bind(tokenHash, nowIso()).first();
...
async function createCaptcha(env) {
  ...
  await env.DB.prepare("INSERT INTO captcha_challenges (id, question, answer_hash, expires_at, used, created_at) VALUES (?, ?, ?, ?, 0, ?)").bind(id, question, answerHash, addMinutes(5), nowIso()).run();
  return json({ captchaId: id, question, expiresInSeconds: 300 });
}
__name(createCaptcha, "createCaptcha");
async function validateCaptcha(env, captchaId, captchaAnswer) {
  if (!captchaId || captchaAnswer === void 0 || captchaAnswer === null) fail("Captcha wajib diisi.");
...
async function registerUser(request, env) {
  ...
  await env.DB.prepare(`INSERT INTO users (id, name, email, password_hash, password_salt, role, status, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, 'santri', 'active', ?, ?)`).bind(id, name, email, hash, salt, nowIso(), nowIso()).run();
...
async function prayerToday(request, env) {
  ...
  await env.DB.prepare(`INSERT OR REPLACE INTO prayer_times_cache
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(crypto.randomUUID(), locationKey, date, times.imsak, times.fajr, times.sunrise, times.dhuhr, times.asr, times.maghrib, times.isha, "aladhan", nowIso()).run();
  return json({ location, date, source: "aladhan", times });
}
__name(prayerToday, "prayerToday");
...
function mapPrayer(row) {
  return { imsak: row.imsak, fajr: row.fajr, sunrise: row.sunrise, dhuhr: row.dhuhr, asr: row.asr, maghrib: row.maghrib, isha: row.isha };
}
__name(mapPrayer, "mapPrayer");
...
__name(guessAudioExtension, "guessAudioExtension");
...
__name(buildSubmissionAudioUrl, "buildSubmissionAudioUrl");
...
__name(extractSubmissionObjectKey, "extractSubmissionObjectKey");
...
async function saveProgress(request, env) {
  ...
  await env.DB.prepare(`INSERT OR REPLACE INTO memorization_progress
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(id, auth.user.id, body.surah_id, body.ayah_number, body.status || "memorized", body.last_reviewed_at || null, body.strength_score || 60, body.mistake_count || 0, nowIso()).run();
...
__name(getTodayReviews, "getTodayReviews");
async function generateReviews(request, env) {
  ...
  for (const row of results) {
    const exists = await env.DB.prepare(`SELECT id FROM review_schedule WHERE user_id=? AND surah_id=? AND start_ayah=? AND end_ayah=? AND due_date=? AND status='pending'`).bind(auth.user.id, row.surah_id, row.ayah_number, row.ayah_number, date).first();
    if (exists) continue;
    await env.DB.prepare(`INSERT INTO review_schedule (id, user_id, surah_id, start_ayah, end_ayah, due_date, priority, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, 'pending', ?)`).bind(crypto.randomUUID(), auth.user.id, row.surah_id, row.ayah_number, row.ayah_number, date, row.strength_score < 60 ? 1 : 3, nowIso()).run();
    count++;
  }
  return json({ ok: true, created: count });
}
__name(generateReviews, "generateReviews");
async function saveReviewResult(request, env) {
  ...
  await env.DB.prepare("INSERT INTO review_logs (id, user_id, surah_id, start_ayah, end_ayah, result, notes, reviewed_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)").bind(crypto.randomUUID(), auth.user.id, row.surah_id, row.start_ayah, row.end_ayah, body.result || "lancar", body.notes || "", nowIso()).run();
  const nextDue = localDateFromDate(new Date(Date.now() + (body.result === "ulang" ? 1 : 7) * 864e5));
  await env.DB.prepare("INSERT INTO review_schedule (id, user_id, surah_id, start_ayah, end_ayah, due_date, priority, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)").bind(crypto.randomUUID(), auth.user.id, row.surah_id, row.start_ayah, row.end_ayah, nextDue, body.result === "ulang" ? 1 : 3, "pending", nowIso()).run();
  return json({ ok: true });
}
__name(saveReviewResult, "saveReviewResult");
function localDateFromDate(d) {
  return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}-${String(d.getDate()).padStart(2, "0")}`;
}
__name(localDateFromDate, "localDateFromDate");
async function createSubmission(request, env) {
  ...
  await env.DB.prepare(`INSERT INTO submissions
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(id, auth.user.id, teacherId, surahId, startAyah, endAyah, audioUrl, "submitted", note, nowIso()).run();
...
__name(getSubmissionAudio, "getSubmissionAudio");
...
__name(resetAccountData, "resetAccountData");
